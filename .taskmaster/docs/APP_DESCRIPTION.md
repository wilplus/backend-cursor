...
**Exactly 5 statuses:** `warm_up` | `task_block` | `final_task_ready` | `post_questions` | `completed`. Active = first four only.
...
- **409 on recording-upload-url the second time:** If the user (or UI) calls recording-upload-url for **"1"** again after the first warm-up recording already succeeded, the session status is now **`task_block`**, so the backend correctly returns **409** ("Session must be in warm_up for recording-1"). **Fix:** Do not call recording-upload-url again for the same step after a successful recording-1. After POST recording-1 returns 200, call **GET session/status** and set the UI step from `session.status` (step 2 = metric questions). If the client ever receives 409 on upload-url, treat it as "session already advanced": refetch status and show the step that matches `session.status` (do not show a generic error or stay on the recording screen).
