...
- **US law.** The United States is out of scope for this instruction. A separate
  US brief exists (document 05) and is parked until we incorporate: with no
  entity, Illinois BIPA's private right of action would land on a natural
  person. Note this is *not* a statement that we serve Poland only — see the
  18 September correction above. We **do** ask about the **EU/EEA**, which we do
  intend to serve — see ask 5 on Art 9(4).
