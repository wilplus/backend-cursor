...
- **`add_coach_override_fields_to_v2_sessions.sql`** — adds optional admin-review fields on `v2_sessions`: `ai_task_score`, `ai_scoring_justification`, `coach_override_score`, `coach_override_justification`. Run if Railway logs show 42703 missing-column warnings for coach override fields.
- **`add_score_for_display_to_v2_sessions.sql`** — adds canonical score fields on `v2_sessions`: `score_for_display` (0-100), `score_ready_at`, `score_components` (JSONB), plus backfill from existing `score` for completed rows.
