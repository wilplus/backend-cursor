...
--
-- RULE 4 — IDK IS A RESPONSE, NOT A NULL, and WHICH response is settled by
-- founder ruling 2026-08-14: **IDK is the ternary's third value, 'neutral'**
-- — the game's "yes / no / idk" — and it is a VALID PERCEPTUAL RATING of the
-- audio ("ambiguous to judge"). Two of them is a FINDING (SPEC I10): a
-- detector taught to say "uncertain" where humans are uncertain beats one
-- that fakes confidence there, and this is the only place that class can
-- come from.
--
-- `unrateable` is the OTHER thing, and stays other: a TECHNICAL abstention
-- (the rater could not hear/judge the clip as audio — a failure of the
-- artifact, not a reading of the voice). It carries no answer, is never
-- counted as a response, and can never settle anything — exactly how the
-- game's twice-labelled gate already treats it ("two abstentions are not two
-- labels"). This closes the wrinkle PR #389 flagged: perceptually_ambiguous
-- can only ever be built from real perceptual reads.
--
...
WITH counted AS (
    ...
    -- below. RULE 4 (founder 2026-08-14): 'neutral' IS the IDK — a valid
    -- perceptual "ambiguous to judge" — while an unrateable row is a
    -- TECHNICAL abstention and is not a response at all.
    SELECT snippet_id,
           state_id,
           CASE WHEN value = 'neutral' THEN 'idk' ELSE value END AS response
      FROM public.confidence_labels
     WHERE lane IN ('coach', 'game_peer')
       AND self_report = false
       AND unrateable IS NOT TRUE
       AND value IN ('yes', 'no', 'neutral')
), tallied AS (
    ...
    -- Same eligibility as counted: a technical abstention is not an answer
    -- from the owner either.
...
COMMENT ON VIEW public.snippet_label_quorum IS
    ...
    '(rule 3); two IDKs settle as perceptually ambiguous (rule 4 — IDK is '
    'the ternary ''neutral'', a perceptual "ambiguous to judge" per founder '
    'ruling 2026-08-14; unrateable is technical and never counted). Twin of '
