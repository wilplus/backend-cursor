-- 0366 · The optional yes takes the D11 locks.
--
...
-- 0365 put v1's lost preamble back and left v2 alone; this file covers v2.
--
...
-- should too.
--
-- THE REPAIR IS 0327's OWN, AS IN 0365. The lock SQL and marker are v1's
-- entry, byte for byte; only the signature differs
-- (tests/test_d11_writer_markers_survive_the_manifest.py compares them). The
-- loop is 0365's: skip when the marker is present; otherwise insert after
...
BEGIN
 ...
 [
  ...
  IF position(spec->>'marker' IN definition)=0 THEN
   -- Whole lines, newline-sensitive, as 0365 counts them: counting
