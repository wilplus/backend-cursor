...
| **Abort during upload** (partial state) | Recording 1/2 | Idempotency | If the same **storage_path** is submitted again and the session already has that recording, the backend returns **200** with the same payload (no duplicate row). Orphaned files in storage are not cleaned up; frontend can retry or re-mint URL. |
...
**Full setup checklist (Supabase, BFF, frontend):** **docs/HOMEWORK-DIRECT-UPLOAD-SETUP.md**.
