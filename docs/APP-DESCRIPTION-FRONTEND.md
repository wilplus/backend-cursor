...
**Admin 404 for /tasks:** The frontend calls **`/api/admin/tasks`** (not `/api/v2/admin/tasks`). The BFF route **must** live at **`src/app/api/admin/tasks/route.ts`** so that the URL is `/api/admin/tasks`. If you put the file under **`src/app/api/v2/admin/tasks/route.ts`**, the route becomes `/api/v2/admin/tasks` and requests to `/api/admin/tasks` will still return 404. Fix: move or create the route at **`src/app/api/admin/tasks/route.ts`** (and **`src/app/api/admin/tasks/[id]/route.ts`** for PUT/DELETE). Copy from **`docs/frontend-admin-panel/api-routes/tasks-route.ts`**; fix the `getAuth` import path for this location (e.g. `../../getAuth` or your app’s shared auth helper).
