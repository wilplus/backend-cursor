...
**When does "HTTP 404 for /tasks" appear?** The error appears when the frontend **sends a request** to **`/api/admin/tasks`**. There is no tasks page — only the Students page — so the call is coming from code that runs when you open a **student profile** (e.g. `getTasks()` in the profile page load). The pathname is correct (Students); you need to **remove** `getTasks()` (and the tasks request) from the student profile page so that URL is never called. Step-by-step: **`docs/frontend-admin-panel/ADMIN-PANEL-IMPLEMENTATION-STATUS.md`** → section "Frontend: how to stop /api/admin/tasks from being called".

**Admin 404 for /tasks (if you add task UI later):** The frontend would call **`/api/admin/tasks`** (not `/api/v2/admin/tasks`). The BFF route **must** live at **`src/app/api/admin/tasks/route.ts`**. If you put it under **`src/app/api/v2/admin/tasks/route.ts`**, requests to `/api/admin/tasks` still get 404. Copy the route from **`docs/frontend-admin-panel/api-routes/tasks-route.ts`** (it includes **`_debug`** and **`X-BFF-Route`** in responses so you can see in the Network tab whether the BFF ran and what the backend returned). Full step-by-step debugging: **`docs/frontend-admin-panel/DEBUG-404-TASKS.md`**. To verify `/api/admin/*` at all, add **`src/app/api/admin/health/route.ts`** (see **`api-routes/health-route.ts`**); if GET /api/admin/health returns 200, routing is fine and the issue is specific to the tasks path or file.

**Cannot add focus task (only if you add task UI back):** The "Add" action would call **POST /api/admin/tasks** with body **`{ title: "your task text" }`**. The backend accepts title-only and defaults `prompt_text` to the title. You need: (1) BFF route **POST /api/admin/tasks** (see `docs/frontend-admin-panel/api-routes/tasks-route.ts`), (2) the Add button’s handler to call that API with the input value, and (3) on success, add the returned **`task`** to the modal’s task list (or refetch tasks) so the new task appears in the pool. Step-by-step and copy-paste handler: **`docs/frontend-admin-panel/FOCUS-TASKS-MODAL-ADD.md`**.
