...
- **Save:** Saves overrides via **PUT /api/admin/students/:id/overrides** (body: **assigned_post_question_ids** (exactly 3 when set), optionally **assigned_warm_up_task_id**, and other overrides). No focus-task selection in the UI; tasks exist only in the DB.
...
**No tasks in admin UI / 404 for /tasks:** The reference has no focus-tasks UI. To fix 404: remove the "Select Focus Tasks" modal and any getTasks()/createTask() or fetch to /api/admin/tasks. You do not need a BFF route for /api/admin/tasks if you do not show task selection.
