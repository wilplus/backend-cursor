...
unchanged. Migration 0314 relaxes only the catalogue's positive-count check
to permit zero. The v2 snapshot derives items in the database; it accepts no
...
- the current acquisition principal for acoustic features;
...
observations from another acquisition principal are excluded, not legalized
by the current account's acceptance. Same-speaker dark assignment IDs support
repeat suppression but are not imported user exposure or training evidence.
...
Authorization is checked at entry, after keyed-lock waits and before writing.
Replay also revalidates authority and exact non-deleted audio. The finalizer
...
time. Policy-purpose and receipt/policy consistency use the accepted M3-2
gate; no independent consent registry is introduced.
...
availability metadata is global content. No deletion adapter is activated or
production data deleted in this slice.
...
- M3-3 executable PostgreSQL suite: **37 passed** (including real concurrent
  creation, late-commit exclusion and authority revoked during a lock wait).
- Focused foundation/deletion/contract compatibility suite: **36 passed**.
- `bash scripts/local_ci.sh --no-setup`: **GREEN**; **4,851 passed, 142
  skipped**, 113 subtests. The 142 skips include the 37 PostgreSQL tests run
...
- Final migration SHA-256:
  `a093f2f3f6f304a36e60b102f3cd9bff9f997ab2f45fa1432536e07d33b235f4`.
