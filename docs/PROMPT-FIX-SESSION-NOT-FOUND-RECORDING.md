...
Use this prompt for another LLM or a frontend dev to fix the homework recording flow. **Use only the real backend:** `v2_sessions` table and homework endpoints (`/v2a/homework/session/start`, `/status`, etc.). Do **not** refer to `api/sessions/[sessionId].ts`, `/api/sessions/:id/status`, or a `sessions` table with `state`. Do **not** let the frontend “create” DB rows or “force warm_up”—session creation and state transitions happen via the existing homework endpoints only.
