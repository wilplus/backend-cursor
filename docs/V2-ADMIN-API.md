...
| PUT | `/v2/admin/students/<user_id>/overrides` | Set per-student overrides. Body: `show_exercise_step` (boolean; when false, this student skips the exercise step), `intended_emotion_prompt`, `keywords_prompt`, `emotion_check_question_text`, `assigned_post_question_ids` (UUID[], must be exactly 3 if provided), `assigned_next_exercise_id` (UUID), `assigned_next_task_ids` (UUID[]). |
...
Exercises tab is **list only** — add/edit content. Who sees the exercise step (and which one) is set **per student on their profile** via `show_exercise_step` and `assigned_next_exercise_id`.
