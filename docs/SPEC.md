...
| D29 | **Nativeness is DERIVED, never stored.** `user_settings.profile_native_language` holds the speaker's **L1** (ISO 639-1); nativeness for a recording is `profile_native_language = recordings.transcription_language`. | *"Is this speaker native?"* has no user-level answer — a native Polish speaker is native presenting in Polish and non-native presenting in English, and a boolean would be wrong for one of the two with no way to tell which. Storing L1 is also strictly more informative: it permits stratifying by **language family**, which is the level transfer effects operate at. `prefer_not_to_say` (refusal) and `NULL` (never asked) are **both excluded from DIF strata and kept separable**. The CHECK constrains *shape*, not a language list — a closed enum would reject exactly the speakers the column exists to protect. Migration `0248`. |
...
| D40 | **The three RATE dimensions are measured over ~40 s of stitched snippets; the three LEVELS stay at snippet grain.** `services/rate_windows.py`. | D33 made this necessary; a 60-second test recording made it concrete — four ~15 s snippets, four `insufficient_data` rows, zero usable measurements, and three planning cycles of measurable speech discarded (F-12). **The snippet is the F1 SEGMENTATION unit, not a measurement window.** Which grain a dimension gets comes from `registry.measurable_in_a_snippet()`, so the two partition the live set with no overlap and no gap. **Gap handling is two independent guards**: the denominator is SUMMED SPEECH, never wall-clock, so a pause between takes cannot enter it (two 20 s snippets five minutes apart give 40 s, not 340); and **windows never cross a recording**, because a session spans takes and a rate averaged across two takes measures neither. Sorting is by `(recording_id, start_offset_ms)` — offsets restart per take, so sorting by offset alone would interleave them. Duration-weighting a value that is already a rate is EXACT, not an approximation: the duration-weighted mean of per-snippet wpm is total words over total minutes. `pause_ms` is the one approximation — the exact figure needs a per-snippet pause COUNT, which the pipeline does not store. Appendix F.4.2. |
...
> **FOUNDER RULING 2026-08-15 — the provisional detector is APPROVED for V1; calibration is DEFERRED.** What ships today is `services/snippet_salience.py`: five components off the existing feature blob (`f0_sd` high, `f0_slope` |·|, `f0_mid_end_delta` |·|, `pause_regularity` inverted, `dynamic_db` high) at **equal 0.2 weights, uncalibrated**, ranking silence-cut windows from `audio_metrics.segment_into_snippets`. It diverges from the four v1.0 parameters above on three of four: terminal contour is present but sign-agnostic and not gated to assertions; amplitude-range **collapse** is not detected (salience rewards *high* `dynamic_db`); speaker-relative mean f0 does not enter selection at all (it lives only in `voice_confidence.py`); there is no latency-spike term. **This divergence is accepted, not a bug to fix now.** §3.1 is the reason it is safe: the detector owes RECALL, the two-human quorum (§9.1, superseded para) owes truth, and nothing it flags reaches a user unadjudicated.
>
> **Why deferred rather than done:** calibrating the weights requires a validation sample, and the fence in `snippet_salience.py` forbids drawing that sample from the salience-selected set (`charisma_snippets`) this function produces. So calibration is a separate data-science pilot, not an in-line tune. **The plan of record:** let the F1 loop run, accumulate quorum-settled human labels in `confidence_labels`, and fit the detector on that corpus later. Until that pilot is commissioned, the weights are constants — do not add tuning machinery, and do not treat the score as a finding.
