...
RANKING RUNS ON PRODUCT EVIDENCE ONLY. Machine confidence may enter once as a
delivery term; an explicit professional-coach tag remains the dominant expert
assessment. Blind peer ratings are research data and never affect this live
...
def select_best_per_slide(candidates: Any) -> dict:
    ...
    transcript, audio_ref, take_index, machine_confidence,
...
def _batch_arc_reads(db, sessions):
    """Read snippets for all of an arc's takes in one query when supported."""
...
def build_best_presentation(
    ...
    # Batch the per-take snippet reads. getattr guards keep injected fake dbs
    # (tests) on the per-session fallback below.
    _snips_batch = _batch_arc_reads(db, sessions)
    # Resolve every take's snippets ONCE, up front. On the fake-db fallback
    # this is still exactly one read per session, not two.
    _snips_by_sid = {
    ...
    for sess in sessions:
        ...
        # is deleted and blind peer panels are internal-only: neither may
        # compete with an explicit product decision.
        #
        ...
        for s in snippets:
            ...
            candidates.append({
                ...
                # CONFIDENCE — the machine composite is the automatic product
                # delivery term. Blind peer labels remain internal research
                # evidence and are intentionally absent from this candidate.
