...
- **Phase-1 processing authority is one boundary.** When `PLF1_PROCESSING_AUTHORIZATION_MODE=enforce`, every recording, retry, provider call and accepted policy version must pass the canonical authorization/snapshot/permit path. Do not add route-local consent logic or direct provider clients. Phase-2 corpus, dataset, training, evaluation, promotion and exercise-adequacy paths remain disabled until separately authorized.
