...
WHAT IT WRITES (two files, deliberately separate):
  ...
  <out>/key.csv          — snippet_id, score, band, cues, baseline and version.
                           Keep this closed until the sheet comes back, then
                           join on snippet_id.
...
def main() -> int:
    ...
    with open(key_path, "w", newline="", encoding="utf-8") as fh:
        ...
        w.writerow(["snippet_id", "score", "band", "cues", "baseline",
                    "version", "session_id", "regime"])
        for r in sample:
            w.writerow([r["snippet_id"], r["score"], r["band"], r["cues"],
                        r["baseline"], r["version"], r["session_id"],
                        r["regime"]])
    ...
    # Weighting provenance. A sample that straddles calculation versions is
    # ratable but not directly comparable.
