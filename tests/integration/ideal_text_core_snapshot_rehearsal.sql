...
SELECT public.publish_ideal_text_document_snapshot_v1(
  ...
  repeat('a',64),
  jsonb_build_object(
    'arc_id','50000000-0000-4000-8000-000000000001','version',1,'status','unverified','title','Test',
...
BEGIN
  ...
  FROM public.ideal_text_document_generations WHERE arc_id='50000000-0000-4000-8000-000000000001';
  BEGIN
    ...
    IF (SELECT generation FROM public.ideal_text_document_generations
        WHERE arc_id='50000000-0000-4000-8000-000000000001') IS DISTINCT FROM before_generation
    THEN RAISE EXCEPTION 'non-ready transition advanced generation'; END IF;

    UPDATE public.v2_sessions SET analysis_state='ready'
    WHERE id='40000000-0000-4000-8000-000000000001';
    IF (SELECT generation FROM public.ideal_text_document_generations
        WHERE arc_id='50000000-0000-4000-8000-000000000001') IS DISTINCT FROM before_generation+1
    THEN RAISE EXCEPTION 'ready transition did not advance generation'; END IF;
    IF public.read_ideal_text_document_core_v1(
      '50000000-0000-4000-8000-000000000001','10000000-0000-4000-8000-000000000001') IS NOT NULL
...
VALUES('50000000-0000-4000-8000-000000000001','Changed source.');
...
DO $$ BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE arc_id='50000000-0000-4000-8000-000000000001' AND generation=1
...
DO $$ BEGIN
  ...
  IF EXISTS (
    SELECT 1 FROM public.list_pending_ideal_text_document_publications_v1(10)
    WHERE arc_id='50000000-0000-4000-8000-000000000001'
