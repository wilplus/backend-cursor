...
Run: python3 -m unittest tests.test_life_panel
...
def _suspend_rate_limits(case: unittest.TestCase) -> None:
    """The limiter is process-global: once any app in the run has called
    init_app, every ``@heavy_limit`` view enforces the per-user budget for
    the rest of the process, whichever Flask app serves it. The route
    classes here build their own app and make more requests than that
    budget allows, so their verdicts depended on collection order (surfaced
    by the audit Q-T9 move, 2026-09-14). flask-limiter exempts every limit
    while ``initialized`` is False; suspend it for this test only and put it
    back afterwards — test_rate_limits.py owns the limiter's own contract."""
    from services.rate_limits import limiter
    if not hasattr(limiter, "initialized"):
        return
    prior = limiter.initialized
    limiter.initialized = False
    case.addCleanup(setattr, limiter, "initialized", prior)
