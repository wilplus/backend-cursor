...
weighting left old and new scores competing in the same arc on different
scales. Two halves:
...
problem:
  ...
  * the backfill and recorder use the same universal calculation, so a
    backfilled piece and a freshly recorded one are comparable;
...
Run: python3 -m unittest tests.test_vc_backfill
"""
...
class _FakeDB:
    ...
    def __init__(self, n=10, stamped_version=None):
        self.snips = [
...
class BackfillTests(unittest.TestCase):
    ...
    def test_the_universal_stamp_contains_no_demographic_routing(self):
        db = _FakeDB(stamped_version="voice-confidence-v1")
        _run(db, apply=True)
        for _sid, blob in db.writes:
            self.assertNotIn("sex", blob["voice_confidence"])
            self.assertNotIn("sex_source", blob["voice_confidence"])
...
class BackfillMatchesTheRecorderTests(unittest.TestCase):
    """Backfilled and freshly recorded pieces use one versioned contract."""

    def test_both_use_the_universal_voice_confidence_module(self):
        ...
        self.assertIn("resolve_confidence_baseline", script)
        self.assertIn("enrich_canonical_pieces", recorder)
        self.assertIn("attach_voice_confidence", recorder_stage)
        for source in (script, recorder_stage):
            self.assertNotIn("resolve_take_sex", source)
            self.assertNotIn("speaker_sex", source)
    ...
    def test_the_backfill_cannot_touch_training_imports(self):
        ...
        snippets directly, and must never invent speaker demographics."""
        with open("scripts/backfill_voice_confidence.py",
                  encoding="utf-8") as fh:
            script = fh.read()
